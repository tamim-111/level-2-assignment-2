...
const getAllBookings = async (req, res) => {
    try {
        ...
        if (user.role === "admin") {
            ...
            const enhanced = await Promise.all(result.rows.map(async (booking) => {
        ...
        if (user.role === "customer") {
            ...
            const enhanced = await Promise.all(result.rows.map(async (booking) => {
                const vehicleRes = await pool.query("SELECT vehicle_name, registration_number, type FROM vehicles WHERE id=$1", [booking.vehicle_id]);
                return {
                    ...booking,
                    vehicle: vehicleRes.rows[0]
                };
...
const updateSingleBooking = async (req, res) => {
    try {
...
//# sourceMappingURL=bookings.controllers.js.map